/**
 * Achievement badge image map and metadata types.
 *
 * ACHIEVEMENT_BADGES maps each achievement ID to its bundled badge asset so
 * the image is available offline and without a CDN round-trip.
 *
 * ACHIEVEMENT_CATEGORIES provides display labels for grouping badges in the UI.
 */

/** Maps achievement IDs to their bundled badge images. */
...
/** Metadata for a single achievement; may be fetched from the server or kept client-side. */
export interface AchievementMeta {
  id: string;
  name: string;
  description: string;
  category: string;
  /** Fallback emoji displayed when the badge image asset is unavailable. */
  icon: string;
}
